...
const PendingDeposits = () => {
  ...
  const fetchDeposits = async () => {
    try {
      const response = await axios.get('https://jizbankplc.com/api/get_pending_deposites.php');
...
const approveDeposit = async (id, userId, amount) => {
  ...
  if (result.isConfirmed) {
    try {
      const response = await axios.post('https://jizbankplc.com/api/approve_deposites.php', { id });
      if (response.data.success) {
...
const notifyUserDepositApproved = async (userId, amount) => {
  try {
    ...
    const response = await fetch('https://jizbankplc.com/api/notify_user_deposit_approved.php', {
      method: 'POST',
      headers: {
 ...
 const deleteTransaction = async (transactionId, userId, amount) => {
    ...
    const handleNotify = async (userId, amount) => {
      ...
      try {
        ...
        const response = await fetch('https://jizbankplc.com/api/notify_user_deposit_declined.php', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
    ...
    <Container>
      ...
      {deposits.map((deposit) => (
        <Card key={deposit.id}>
          ...
          <p 
          onClick={()=>window.open(`https://jizbankplc.com/api/${deposit.screenshot_path}`, '_blank')}
